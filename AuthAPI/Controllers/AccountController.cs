    ...
    public class AccountController : ControllerBase
    {
       ...
       roleManager, IConfiguration configuration)
        ...
        public async Task<ActionResult<AuthResponseDto>> Login(LoginDto loginDto)
        {
            ...
            user.RefreshTokenExpiredTime = DateTime.UtcNow.AddMinutes(RefreshTokenValidityIn);
        ...
        public async Task<ActionResult<AuthResponseDto>> RefreshToken(TokenDto tokenDto)
        {
            ...
            if (principal is null || user is null || user.RefreshToken != tokenDto.RefreshToken || user.RefreshTokenExpiredTime <= DateTime.UtcNow)
            {
        ...
        private ClaimsPrincipal? GetPrincipalFromExpiredToken(string token)
        {
            ...
            var principal = tokenHandler.ValidateToken(token,tokenParameter, out SecurityToken securityToken);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordDto forgotPasswordDto)
        {
            ...
            //request.AddParameter("application/json", "{\"from\":{\"email\":\"hello@demomailtrap.co\",\"name\":\"Mailtrap Test\"},\"to\":[{\"email\":\"ian.aquinoflores@gmail.com\"}],\"template_uuid\":\"df58fac7-912b-44c7-8c8e-f2313ff1bfff\",\"template_variables\":{\"user_email\":\"Test_User_email\",\"pass_reset_link\":\"Test_Pass_reset_link\"}}", ParameterType.RequestBody);
            ...
            request.AddHeader("Authorization", "Bearer d7928f566cbe9143576a9f4a98c0993e");
            request.AddJsonBody(new
            {
                from = new { email = "mailtrap@demomailtrap.co" },
                to = new[] { new { email = user.Email } },
                template_uuid = "df58fac7-912b-44c7-8c8e-f2313ff1bfff",
            ...
            if (response.IsSuccessful)
            {
                ...
                {
                    IsSuccess = true,
                    Message = "Email sent with password reset link. Please check youre email."
                });
            }
            else
            {
                return BadRequest(new AuthResponseDto { IsSuccess = false, Message = response.Content!.ToString() });
